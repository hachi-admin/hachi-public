...
test('L5 failed regeneration keeps the original body and exposes terminal status', async () => {
  const requests = []; let draftReads = 0; const router = (url) => { const path = String(url); requests.push(path); if (path.endsWith('/exchange')) return json({ token: jwt() }); if (path.endsWith('/context')) return json({ accounts: [{ accountId: 'a1', label: 'A' }], member: { role: 'member' } }); if (path.includes('/settings')) return json({ settings: { revision: 0, profile: {}, templateRefs: [] } }); if (path.includes('/budget')) return json({ operation: { limitMicroJPY: 10000000 } }); if (path.includes('/regenerate')) return json({ job: { jobId: 'j-failed', status: 'queued' } }); if (path.includes('/generation-jobs/j-failed')) return json({ job: { jobId: 'j-failed', status: 'failed', errorCode: 'GENERATION_OUTPUT_INVALID', providerDiagnostic: { code: 'INVALID_ARGUMENT', message: 'Request contains an invalid argument.' } } }); if (path.includes('/drafts?')) return json({ drafts: [{ draftId: 'd1', generationGroupId: 'g', variantId: 'v1', state: 'needs_review', revision: draftReads++, body: '元本文', validation: { ok: true } }] }); return json({}); };
  const dom = page('#x_code=regen-failed', true, router, { verifier: 'regen-failed-v' }); await flush(); await selectFirstAccount(dom); await flush(); const form = dom.window.document.querySelector('.x-regeneration-form'); form.elements.instruction.value = '短くする'; form.querySelector('[data-regeneration-action]').click(); await flush(); await flush(); await flush(); const text = dom.window.document.querySelector('#x-drafts').textContent; assert.equal(dom.window.document.querySelector('#x-drafts textarea').value, '元本文'); assert.match(text, /再生成ジョブ: failed/); assert.match(text, /j-failed/); assert.match(text, /GENERATION_OUTPUT_INVALID/); assert.match(text, /INVALID_ARGUMENT/); assert.match(text, /Request contains an invalid argument/); assert.ok(requests.some(path => path.includes('/generation-jobs/j-failed')));
});
